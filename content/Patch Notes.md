...
## March 2024
### New Additions
- Added [[ladder#`/ladder info`|/ladder info]] command to show leaderboard statistics.
- Added Swiss Tourney system to PvP leaderboards.
### Updates
- You can now move [[rolereact]]s with [[rolereact#`/rolereact move`|/rolereact move]] 
- You can now move  [[ticketer]]s with [[ticketer#`/ticketer move`|/ticketer move]] 
- [[ladder#`/ladder player_override force`|/ladder player_override force]] now creates a thread on the created challenge.
- [[ladder#`/ladder info`|/ladder info]] will account for reset times by default, and ignore matches before resets if the time range includes a reset.
...
- [[slugboard#`/slugboard create`|/slugboard create]] now takes an initial emoji flag parameter.
- [[ladder#`/ladder info`|/ladder info]] now displays number of unique players in the given period.
- [[ladder#`/ladder info`|/ladder info]] now displays top three greatest Elo improvements.
### Bug Fixes
- Fixed [[leaderboard|/leaderboard activity weekly]] being shown when weekly activity is disabled.
- Fixed activity Exp not updating on profiles.
- Fixed bug in [[blackjack]] where draws were displayed as wins.
- Fixed bug where the wrong emoji was listed in [[leaderboard|/leaderboard slugboard]].
- Fixed bug causing [[userinfo|/userinfo]] to fail when an invalid user ID was given.
- Fixed bug causing [[blackjack]] to not pay out on getting blackjack.
- Fixed bug causing no persistent roles to be listed in [[persistentrole#`/persistentrole list`|/persistentrole list]].
...
## January 2024
### New Additions
- Added platform limit option to [[challenge|/challenge]].
### Updates
- Added `Remove Reminder` button to [[reminder#`/reminder list`|/reminder list]].
- Added `always_send` option to [[reminder#`/reminder add`|/reminder add]], allowing reminders to be force-sent even if the previous reminder was within the last few messages in that channel.
...
- Fixed incorrect [[scan|/scan]] prompt in `STEAL` warning DM.
...
- Fixed [[elo|/elo]] command not display past matches for teams.
